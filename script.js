...
function deleteToDoItem(todoIndex){
    allTodos = allTodos.filter((_, i) => i !== todoIndex); //returns true if i !== todoIndex (task kept if true, removed if false)
    saveTodos();
    updateTodoList();    
}
function saveTodos(){
    const todosJson = JSON.stringify(allTodos); //convert array to string
    localStorage.setItem("todos", todosJson);
}
function getTodos(){
    const todos = localStorage.getItem("todos") || "[]";
    return JSON.parse(todos); //convert string back to array
}